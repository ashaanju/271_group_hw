...
```{r}
summary(d) 
#sanders_preference has 9 null values (binary)
#party is categorical
#race is binary
#gender is 1/2
#max age looks reasonable 1997 can vote 
describe(d)
nrow(d) #1200 lines
cor(d,d) #party/race_white and race_white/birthyr have ~ high cor

```

```{r}
d$fparty <- factor(d$party)
```
...
```{r}
model1 <- glm(d$sanders_preference ~ d$birthyr, family = binomial (link = logit))
summary(model1)
```

```{r}
model2 <- glm(d$sanders_preference ~ d$birthyr + d$fparty, family = binomial (link = logit))
summary(model2)
```

```{r}
model3 <- glm(d$sanders_preference ~ d$birthyr + d$fparty + d$race_white, family = binomial (link = logit))
summary(model3)
```
```{r}
model4 <- glm(d$sanders_preference ~ d$birthyr + d$fparty + d$race_white + d$gender, family = binomial (link = logit))
summary(model4)
```
